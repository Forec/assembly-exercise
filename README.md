...
* 41 ~ 73 题：选自[中山市飞蛾软件工作室](http://www.feiesoft.com/asm/)，此部分习题的实现可参考以下资料：
...
编写程序，检测计算机是否已安装了鼠标，并以显示Yes/No来表示检测结果。

* 55、编写程序，显示鼠标的按键，若按左键，显示 `Left` ，若按右键，显示 `Right` ，按 `Esc` 键，程序结束。

* 56、编写一个程序，在图形模式下，打开鼠标，并显示鼠标的位置。

* 57、编写一个程序，显示驱动器 C 根目录下所有 EXE 文件（提示：用通配符"*.exe"来查找与此匹配的文件名，找到后，显示之）。

* 58、编写一个程序，显示驱动器 C 根目录下的所有子目录。

* 59、编写一个程序，设置指定目录下的所有文件为只读文件。

* 60、编写一个程序，创建一个隐含文件 `Data.TXT`。

* 61、编写一个程序，把键盘上输入写入文件 `File.dat` 中，类似以下 DOS 命令：
```
COPY con File.dat
```

* 62、编写一个宏，它产生 n 条 NOP 指令，其中n是宏的形式参数。

* 63、编写只有一个形式参数的宏PRINT，其具体功能如下（提示：用 IFB 或 IFNB 语句来测试是否有参数）：
 * 若引用时带有参数，则在屏幕上显示其参数字符，如：`PRINT 'A'`，则显示字符 'A'；
 * 若引用时不带实参，则显示回车和换行，如：`PRINT`。

* 64、编写一个在屏幕上连续显示 `Welcome…` 的信息，要求显示颜色和显示位置都是随机的。在新位置显示时，原位置的信息抹去。当按任意键时，程序结束运行。

* 65、从键盘输入一个表示年份的正整数（1 ~ 65535），然后判断其是否为闰年。若是，则输出 `Yes`，否则，输出 `No`。

* 66、编写一个带命令行参数的程序 Words ，输出指定正文文件中的单词，假设单词为连续的字母串。比如：`words file.txt`，显示文件 `file.txt` 中的每个单词

* 67、编写一个建立双向链表的程序，每接受一个整数，链表增加一个结点，当遇到负数时，结束链表结点的增加，然后从表尾向前输出各结点中的数值。

* 68、编写一个带命令行参数的程序 Calc ，其命令行参数是一些整数，程序输出它们之和。如果某参数为非法整数，则忽略之。比如：`Calc 120 +34 -56`，则程序显示 98。

* 69、在 68 的基础上，在不考虑运算符优先级的情况下，实现一个可进行算术四则运算（只有加、减、乘和除，没有括号等）的计算器。

* 70、编写一个简单的加密程序，它把一个文件中的所有字母字符按下列规律进行转换，非字母的字符保持不变。转换规律：'A' → 'Z'，'B' → 'Y'，……。

* 71、编写一个程序，它可显示 BMP 的图形文件。

* 72、编写一个程序，它可显示 JPG 的图形文件。

* 73、编写一个程序，求出 2 ~ 100 之内的所有素数。
